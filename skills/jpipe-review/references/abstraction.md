...
- *"the committed Pipfile and the pipeline source files"* — **two facts.** An allowlist comparison and
  an import scan share nothing but the word "and".
...
| any leaf that would make a reasonable `conclusion` | — |
...
`jpipe-tutorial-2026/justifications/requirements/r9.jd` — two `fact → check → verdict` legs combined
by a top warrant. This is what "at the right abstraction" looks like in full.
...
— cited in the file header as SPECS §3.3, not argued), and no **rebuttal** (nothing addresses a
